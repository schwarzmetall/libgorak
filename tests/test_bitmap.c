...
static void test_init_dont_touch(void)
{
    ...
    int_fast8_t s = lgk_bitmap_init(&bm, buf, BITS, -1);
    test_assert(s == 0);
    test_assert(bm.map[0] == 0x55);
    test_assert(bm.map[1] == 0xaa);
    test_assert(bm.map[2] == 0x0f);
    test_assert(bm.map[3] == 0xf0);
}
...
static void test_set_get(void)
{
    ...
    for (unsigned i = 0; i < BITS; i++) {
        ...
        test_assert(set_ret == 1);
        test_assert(lgk_bitmap_get(&bm, i) == 1);
    }
    for (unsigned i = 0; i < BITS; i++) {
        int_fast8_t set_ret = lgk_bitmap_set(&bm, i, 0);
        test_assert(set_ret == 0);
        test_assert(lgk_bitmap_get(&bm, i) == 0);
    }
}
...
static void test_get_out_of_bounds_returns_error(void)
{
    ...
    lgk_bitmap_t buf[4];
    lgk_bitmap_init(&bm, buf, BITS, 0);
    test_assert(lgk_bitmap_get(&bm, BITS) == -1);
    test_assert(lgk_bitmap_get(&bm, BITS + 1) == -1);
}
...
static void test_set_out_of_bounds_returns_error(void)
{
    ...
    lgk_bitmap_t buf[4];
    lgk_bitmap_init(&bm, buf, BITS, 0);
    test_assert(lgk_bitmap_set(&bm, BITS, 1) == -1);
    test_assert(lgk_bitmap_set(&bm, BITS + 1, 0) == -1);
}
...
static void test_get_zero_size_returns_error(void)
{
    ...
    bm.map = NULL;
    bm.size = 0;
    test_assert(lgk_bitmap_get(&bm, 0) == -1);
}
